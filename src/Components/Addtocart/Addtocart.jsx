...
function Addtocart() {
    ...
    const totalprice=Math.ceil(context.gettotalproce() + parseInt(selectedop) - parseInt(context.promocode))
    ...
    //   const navi = useNavigate()
      ...
      const handleSubmit = (event) => {
        ...
        //   addDoc(collection(db, "customer details"), {
    ...
    return (
        <>
           {context.Cart.length<1?<> <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="text-center p-6 bg-white rounded-lg shadow-md">
        ...
        <h1 className="text-5xl font-bold text-gray-800 mb-4">Your Cart is Empty</h1>
        ...
        <Link to="/Allproduct" className="bg-blue-500 text-white py-2 px-6 rounded-full font-semibold hover:bg-blue-600 transition duration-300 ease-in-out">
          Continue Shopping
                ...
                <div className="flex  flex-col md:flex-row  shadow-md my-10">
                    <div className="md:w-3/4 bg-white px-10 py-3 w-auto ">
                        <div className="flex justify-between border-b pb-5">
                            <h1 className="font-semibold text-2xl">Shopping Cart</h1>
                            <h2 className="font-semibold text-2xl">{context.Cart.length}</h2>
                        </div>
                        ...
                        <div>
                            {context.Cart.map((dd) => (
                                <div key={dd.id} className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                                    <div className="flex w-2/5">
                                        <div className="w-20">
                                         <Link to="/Singlepage" onClick={() => context.handlesiglepage(dd)}><img className="h-26 bg-cover" src={dd.image} alt="" /></Link>   
                                        </div>
                                        <div className="flex flex-col justify-between ml-4 flex-grow">
                                            <span className="font-bold text-sm">{dd.title}</span>
                                            <span className="text-blue-500 text-xs">{dd.category
                                            }</span>
                                    ...
                                    <div className="flex justify-center w-1/5">
                                        <button className=" text-black w-3 text-3xl shadow-lg  w-fit p-1"
                                            onClick={() => context.handledec(dd.id)}>
                                            - </button>

                                        <input className="mx-2 border text-center w-8" type="text" value={dd.quantity} readOnly />

                                        <button className=" text-black w-3 text-2xl shadow-lg  p-1"
                                            onClick={() => context.handleinc(dd.id)}>
                                            + </button>
                                    </div>
                                    <span className="text-center w-1/5 font-semibold text-sm">${dd.price}</span>
                                    <span className="text-center w-1/5 font-semibold text-sm">${dd.price * dd.quantity}</span>
                                </div>
                    ...
                    <div id="summary" className="w-1/4 px-8 w-auto py-4">
                        <h1 className="font-semibold text-2xl border-b pb-8">Order Summary</h1>
                        <div className="flex justify-between mt-10 mb-4">
                            <span className="font-semibold text-sm uppercase">{context.Cart.length}</span>
                        ...
                        <div>
                            ...
                            <option >choose shipping options</option>
                                ...
                                <option value="20">primeame shipping - $20.00</option>
                            </select>
                        </div>
                        <div className="py-10">
                            ...
                            <input
                                ...
                                type="text" id="promo" placeholder="Enter your code" className="p-2 text-sm w-full" />
                        </div>
...
<div className="container mx-auto p-6">
      <h1 className="text-4xl font-bold mb-6">Place Your Order</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* <button type="submit" className="w-full bg-blue-500 text-white py-2 px-4 rounded-md font-semibold hover:bg-blue-600">Place Order</button> */}
      </form>
    </div>
                           
                            <Link  onClick={handlecheckout} className="bg-indigo-500  font-semibold hover:bg-indigo-600 py-3 px-3 text-sm text-white uppercase w-full">Checkout</Link>
                        </div>
                    </div>
